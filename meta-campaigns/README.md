...
Tento fallback je fail-closed. Preflight načte stránky jednou přes User-token edge `/me/accounts` a vyžaduje u obou očekávaných Page ID úlohu `ADVERTISE` a nulové hodnoty `instagram_business_account` i `connected_instagram_account`. Současně vyžaduje prázdný Marketing API edge `act_<id>/instagram_accounts`. Pokud stránka v `/me/accounts` chybí nebo se později objeví propojený Instagram účet, skript se zastaví před zápisem, aby jeho identitu musel člověk nejdříve ověřit a schválit v plánu.
